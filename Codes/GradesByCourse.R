...
Grades$semester <-
  paste0(substr(Grades$TermCode, 1, 4),
         "-",
         substr(Grades$TermCode, 5, 6))

attach(Grades)
AdjGrades = data.frame(cbind(
  subset(Grades),
  A = (Grades[, "Tot_A"] + Grades[, "Tot_Aplus"] +
         Grades[, "Tot_Aminus"]),
  B = (Grades[, "Tot_B"] + Grades[, "Tot_Bplus"] +
         Grades[, "Tot_Bminus"]),
  C = (Grades[, "Tot_C"] + Grades[, "Tot_Cplus"] +
         Grades[, "Tot_Cminus"]),
  D = (Grades[, "Tot_D"] + Grades[, "Tot_Dplus"] +
         Grades[, "Tot_Dminus"]),
  F_ = (Grades[, "Tot_F"]),
  W = (Grades[, "Tot_W"]),
  WF = (Grades[, "Tot_WF"])
...
for (kk in 1:length(class)) {
  ...
  ################
  pdf(
    file = paste0(Course[1, "SubjNumb"], "_", Type, ".pdf"),
    width = 12,
    height = 7
  )
  
  #DFW graphs
  DFW_T = aggregate(cbind(Tot_Enr, DFW) ~ TermCode,
                    data = Course,
                    FUN = sum)
  ...
  mtext(
    text = paste(Course[1, "SubjNumb"], Type, "trend", title),
    line = -2,
    cex = 1.4,
    outer = T
  )
  ...
  mtext(
    text = paste(Course[1, "SubjNumb"], "Grade Distribution", title),
    line = -2,
    cex = 1.4,
    outer = T
  )
